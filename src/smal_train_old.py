...
# TODO: pass as flags
CONFIG_DIR = "configs/"
CONFIG_MODEL = "train_params_cows.json"
...
class ShapeTrainer(train_utils.Trainer):
    def define_model(self):
        ...
        texture_mask_path = opts["dataset"] + "_" + opts["texture_png"]
        self.texture_map_mask = torch.Tensor(
            imageio.imread(texture_mask_path) / 255.0
        ).cuda(device=opts["gpu_id"])
        tex_masks = None

        data_path = os.path.join(opts["model_dir"], opts["data_name"])
        ...
        # if self.model.edges2verts is not None:
        ...
        # TODO: revert to having smal in MeshNet?
        self.smal = SMAL(self.opts)
    ...
    def init_dataset(self):
        ...
        elif opts["dataset"] == "cows":
            self.data_module = cows_data
        else:
        ...
        if "model_trans" in batch.keys():
            ...
            if opts["use_norm_f_and_z"]:
                model_trans_tensor[:, 2] = (
                    model_trans_tensor[:, 2] - opts["norm_z"] + 1.0
                )
    ...
    def set_texture_map(self, batch):
        if opts["texture_map"]:
            ...
            self.texture_map = Variable(
                texture_map_tensor.cuda(device=opts["gpu_id"]), requires_grad=False
            )
        else:
    ...
    def set_input(self, batch):
        # Image with annotations.  TODO: why both?
    ...
    def forward(self):
        ...
        if self.opts["texture"]:
            ...
            pred_codes, _ = self.model.forward(self.input_imgs, masks)
        ...
        self.cam_pred = torch.cat(
            [
                self.scale_pred[:, [0]],
                torch.ones(opts["batch_size"], 2).cuda()
                * self.input_imgs.shape[-1]
                / 2,
            ],
            dim=1,
        )
        ...
        if opts["only_mean_sym"]:
            del_v = self.delta_v
        elif opts["use_delta_v"]:  # TODO: correct? (unused in my version)
            del_v = self.model.symmetrize(self.delta_v)

        if opts[
            "no_delta_v"
        ]:  # TODO: somehow merge the parameters no_delta_v and use_delta_v
            del_v[:] = 0

        if opts["use_smal_pose"]:
            # TODO: revert to using self.model.get_smal_verts
        ...
        synth_rgb, synth_silhouettes = self.renderer(
            verts, faces, self.cam_pred, self.opts["gpu_id"]
        )
        ...
        self.kp_verts = torch.matmul(self.vert2kp, verts)
        ...
        # verts, joints, _ = self.model.smal(
        #     self.betas_pred, self.pose_pred, trans=self.trans_pred, del_v=self.delta_v
        # )
        # faces = self.model.smal.faces.unsqueeze(0).expand(verts.shape[0], 7774, 3)
        # self.renderer.directional_light_only()
        # synth_rgb = (self.renderer(verts, faces, self.cam_pred).cpu().detach().numpy())[
        #     0, ...
        # ]
        # print("SYNTH:", synth_rgb.shape)
        # imageio.imwrite("example_in_imageio.jpg", synth_rgb)

        # example = self.input_imgs[0, ...].cpu().detach().numpy().reshape((3, 256, 256))

        # print(self.betas_pred.shape, self.pose_pred.shape, "!!!")
        # print(self.pred_v.shape, self.vert2kp.shape, self.kp_verts.shape, "!")
        # # synth_rgb = (
        # #     self.renderer(
        # #         self.kp_verts[0, ...].reshape((1, -1, 3)), faces, self.cam_pred
        # #     )
        # #     .cpu()
        # #     .detach()
        # #     .numpy()
        ...
        # example_kp = self.kps[0, ...].cpu().detach().numpy().reshape((30, 3))
        # example_kp_pred = self.kp_pred[0, ...].cpu().detach().numpy().reshape((30, 2))
        # import matplotlib.pyplot as plt

        # fig, ax = plt.subplots()
        # ax.imshow(example, vmin=0, vmax=1)
        # ax.scatter(
        #     example_kp[example_kp[:, 2] > 0, 0],
        #     example_kp[example_kp[:, 2] > 0, 1],
        #     color="green",
        #     s=3,
        # )
        # ax.scatter(
        #     example_kp_pred[example_kp[:, 2] > 0, 1],
        #     example_kp_pred[example_kp[:, 2] > 0, 0],
        #     color="red",
        #     s=3,
        # )
        # fig.savefig("example_in.jpg")
        # return
        ...
        if opts["use_pose_prior"]:
            pose_prior = Prior(opts["walking_prior"])
            self.pose_prior_loss = torch.mean(pose_prior(self.pose_pred))
            self.total_loss += opts["pose_prior_wt"] * self.pose_prior_loss
    ...
    def get_current_visuals(self):
        ...
        for i in range(num_show):
            ...
            rends = np.hstack((rend_predcam, rend_gtcam))
            if self.masks is not None:
                ...
                vis_dict["%d" % i] = np.hstack((imgs, rends))
    ...
    def get_current_scalars(self):
        ...
        if self.opts["use_pose_prior"]:
            sc_dict["pose_prior"] = self.pose_prior_loss.item()
        if (
