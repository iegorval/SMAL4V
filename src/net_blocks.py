...
def fc(norm_type, nc_inp, nc_out):
    if norm_type == "batch":
        ...
        return nn.Sequential(nn.Linear(nc_inp, nc_out), nn.LeakyReLU(0.1, inplace=True))
...
def decoder2d(
    ...
    for nl in range(nlayers):
        ...
        if use_deconv:
            ...
            nc_input = nc_output
            modules.append(
                conv2d(norm_type, nc_input, nc_output, num_groups=num_groups // 2)
            )
        else:
    ...
    modules.append(
        nn.Conv2d(nc_output, nc_final, kernel_size=3, stride=1, padding=1, bias=True)
    )
...
## 3D Network Modules
def encoder3d(
    nlayers, norm_type="batch", nc_input=1, nc_max=128, nc_l1=8, nc_step=1, nz_shape=20
):
    """Simple 3D encoder with nlayers.
...
def decoder3d(
    ...
    modules.append(
        nn.Conv3d(nc_output, nc_final, kernel_size=3, stride=1, padding=1, bias=True)
    )
    decoder = nn.Sequential(*modules)
    net_init(decoder)
    return decoder
