...
def sample_texture(uv_map, uv_img):
    ...
    textures = torch.nn.functional.grid_sample(
        uv_img_t, uv_map_t.view(1, F, T * T, 2), align_corners=True
    )
...
def make_symmetric(verts, faces, left_inds, right_inds, center_inds):
    ...
    new_faces, num_indept_faces, num_sym_faces = make_faces_symmetric(
        new_verts, new_faces0, num_indept, num_sym
    )
...
def make_faces_symmetric(verts, faces, num_indept_verts, num_sym_verts):
    ...
    left_ids = np.arange(
        num_indept_verts + num_sym_verts, num_indept_verts + 2 * num_sym_verts
    )
    ...
    verts2faces = [
        np.where((faces == v_id).any(axis=1))[0] for v_id in range(verts.shape[0])
    ]
    ...
    for f_id in range(faces.shape[0]):
        ...
        if sorted(v_sym_ids) == sorted(v_ids):
            ...
            if np.all(
                verts[face_here][unique_vids, 0] < verts[sym_face_here][unique_vids, 0]
            ):
                # f_id is left
...
def compute_edges2verts(verts, faces):
    ...
    result = np.stack(
        [np.hstack((edge, other_vs)) for edge, other_vs in edge_dict.items()]
    )
    return result
...
def compute_vert2kp(verts, mean_shape):
    ...
    dists = np.stack(
        [np.linalg.norm(verts - verts[nn_ind], axis=1) for nn_ind in nn_inds]
    )
    vert2kp = -0.5 * (dists) / 0.01
    return vert2kp
