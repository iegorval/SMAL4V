...
class Visualizer:
    def __init__(self, opts):
        ...
        if self.display_id > 0:
            self.vis = visdom.Visdom(port=opts["display_port"])
            self.display_single_pane_ncols = opts["display_single_pane_ncols"]

        self.log_name = os.path.join(
            opts["checkpoint_dir"], opts["name"], "loss_log.txt"
        )
    ...
    def plot_current_scalars(self, epoch, counter_ratio, opts, scalars):
